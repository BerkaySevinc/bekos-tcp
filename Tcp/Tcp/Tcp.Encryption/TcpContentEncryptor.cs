...
// yanlış key varsa atılan exceptionu handlela


internal class TcpContentEncryptor : IDisposable
{

    public TcpContentEncryptor(TcpEncryptionOptions options) =>
        ConfigureEncryption(options);
    public TcpContentEncryptor() : this(new TcpEncryptionOptions()) { }
    ...
    public byte[] Encrypt(byte[] data) => Encryptor.Invoke(data);
    public byte[] Decrypt(byte[] cipherData) => Decryptor.Invoke(cipherData);
    ...
    private byte[] DecryptWithIV(byte[] data)
    {
        ...
        using var aesEncryptor = aes.CreateDecryptor(aes.Key, iv);
        using var cryptoStream = new CryptoStream(memoryStream, aesEncryptor, CryptoStreamMode.Write, true);
    ...
    private static byte[] GetEncryptionKey(TcpEncryptionOptions options)
    {
